...
The ≈ mark appears in the **Unoccupied Beds**, **Actual Bed Capacity**, and **Occupancy Rate** cells for emergency bed programs (Mixed Adult, Men, Women, Youth emergency; and their totals). On May 14, 2026, the total gap was **62 beds** across those programs — 2.2% of the City's stated emergency bed capacity of 2,810. BonQuery's table now shows the funded figure inline next to the actual figure (in grey, parenthesised), so the gap is reconstructible from the BQ data alone: `funded − actual = offline`.

This is not an error in either source. It is a definitional difference between two City data products:

- **CKAN open-data export** (`CAPACITY_ACTUAL_BED`): counts beds physically set up and bookable for service users on the date of recording. The CKAN data dictionary explicitly notes this excludes beds taken out of service for **maintenance, repair, or service users in self-isolation**.
- **City's live dashboard** ("Actual Bed Capacity"): reports the funded capacity (`CAPACITY_FUNDING_BED`) — i.e., the contractually funded number of beds, including any that are currently offline.
...
Both figures are valid. They answer different questions: the City's "Actual Bed Capacity" reports what the system is funded to provide; BonQuery's `cap` column reports what was actually available that day. BonQuery uses the CKAN open-data definition because that is the published, reproducible figure; the funded figure is now shown alongside it for full reconciliation.
